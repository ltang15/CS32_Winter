...
void City::preachToFlatulansAroundPlayer()
{
    ...
    int unconverted = 0; // initialize number of flatulans unconverted at each time of preaching
    if (m_player == nullptr)
        return;
    ...
    for (int k = 0; k < m_nFlatulans; )
    {
        ...
        {   // conversion succeeds
            ...
            {
                unconverted++; // increment number of unconverted flatulans      
                k++;
            }
        }
        else
    ...
    // at least one flatulan unconverted, then records the position of player
    if (unconverted > 0)
